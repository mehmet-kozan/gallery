      ...
      children: [
        ...
        Expanded(
          ...
          child: Card(
            ...
            margin: EdgeInsets.only(bottom: isDesktop ? 65 : 75),
            child: sidebarItems,
          ),
        ),
      ],
...
class CustomSidebarItem extends StatefulWidget {
  CustomSidebarItem({Key key}) : super(key: key);

  @override
  _CustomSidebarItemState createState() => _CustomSidebarItemState();
}

class _CustomSidebarItemState extends State<CustomSidebarItem> {
        ...
        body: NativeDataTable.builder(
          ...
          selectedActions: <Widget>[
            IconButton(
              ...
              onPressed: () {
                setState(() {
                  for (var item in _items?.where((d) => d?.selected ?? false)?.toSet()?.toList()) {
                    _items.remove(item);
                  }
                });
              },
          ...
          mobileIsLoading: const CircularProgressIndicator(),
          noItems: const Text('No Items Found'),
          columns: <DataColumn>[
            DataColumn(label: const Text('Dessert (100g serving)'), onSort: (columnIndex, ascending) => _sort<String>((d) => d.name, columnIndex, ascending)),
            DataColumn(label: const Text('Calories'), tooltip: 'The total amount of food energy in the given serving size.', numeric: true, onSort: (columnIndex, ascending) => _sort<num>((d) => d.calories, columnIndex, ascending)),
            DataColumn(label: const Text('Fat (g)'), numeric: true, onSort: (columnIndex, ascending) => _sort<num>((d) => d.fat, columnIndex, ascending)),
            DataColumn(label: const Text('Carbs (g)'), numeric: true, onSort: (columnIndex, ascending) => _sort<num>((d) => d.carbs, columnIndex, ascending)),
            DataColumn(label: const Text('Protein (g)'), numeric: true, onSort: (columnIndex, ascending) => _sort<num>((d) => d.protein, columnIndex, ascending)),
            DataColumn(label: const Text('Sodium (mg)'), numeric: true, onSort: (columnIndex, ascending) => _sort<num>((d) => d.sodium, columnIndex, ascending)),
            DataColumn(label: const Text('Calcium (%)'), tooltip: 'The amount of calcium as a percentage of the recommended daily amount.', numeric: true, onSort: (columnIndex, ascending) => _sort<num>((d) => d.calcium, columnIndex, ascending)),
            DataColumn(label: const Text('Iron (%)'), numeric: true, onSort: (columnIndex, ascending) => _sort<num>((d) => d.iron, columnIndex, ascending)),
            const DataColumn(
              label: Text('Actions'),
            ),
          ],
...
//   const SidebarItem({Key key, this.value, this.icon}) : super(key: key);
...
//       ),
//       title: Text(value),
//       subtitle: Text('Some value $value'),
//       trailing: const Icon(
